...
class ModulesController {
     ...
     */
    public function getById($params, $slug) {
        try {
            // Убедимся, что slug - это строка
            if (is_array($slug)) {
                return ['error' => 'Некорректный идентификатор модуля'];
            }
     ...
     */
    public function install($params, $slug) {
        try {
            // Убедимся, что slug - это строка
            if (is_array($slug)) {
                return ['success' => false, 'message' => 'Некорректный идентификатор модуля'];
            }
     ...
     */
    public function activate($params, $slug) {
        try {
            // Убедимся, что slug - это строка
            if (is_array($slug)) {
                return ['success' => false, 'message' => 'Некорректный идентификатор модуля'];
     ...
     */
    public function deactivate($params, $slug) {
        try {
            // Убедимся, что slug - это строка
            if (is_array($slug)) {
                return ['success' => false, 'message' => 'Некорректный идентификатор модуля'];
     ...
     */
    public function uninstall($params, $slug) {
        try {
            // Убедимся, что slug - это строка
            if (is_array($slug)) {
                return ['success' => false, 'message' => 'Некорректный идентификатор модуля'];
